...
function AnalyzeWizard() {
  ...
  // Derive wizard step
  const showDashboard = analysis !== null && metrics !== null && investmentSummary !== null;
  let wizardStep: "upload" | "extracting" | "extracted" | "analyzing" | "analyzed" | "report";
  if (showDashboard) {
    wizardStep = "report";
  } else if (analysis !== null) {
    wizardStep = "analyzed";
  } else if (analyzeLoading) {
    wizardStep = "analyzing";
  } else if (extractResults !== null) {
    wizardStep = "extracted";
  } else if (extracting) {
  ...
  async function handleGenerateReport() {
    ...
    try {
      ...
      if (AUTH_ENABLED && session?.user?.id) {
        const filename = pdfFiles.length > 0 ? pdfFiles[0].name : undefined;
        fetch("/api/analyses", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const canGenerateReport = analysis !== null && metrics !== null && !summaryLoading && !showDashboard;
    ...
    <main className="bg-gray-50 min-h-screen">
      <div className="max-w-5xl mx-auto px-4 py-16">
        {checkoutSuccess && (
          <div className="mb-6 bg-green-50 border border-green-200 rounded-xl p-4">
            <p className="text-green-700 font-medium">
              Subscription activated! You now have Pro access.
            </p>
          </div>
        )}
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-900">Analyze Property</h1>
        ...
        {extractErrors.length > 0 && (
          <div className="mt-8 space-y-2">
            <h2 className="text-xl font-semibold text-red-700">Extraction Errors</h2>
            {extractErrors.map((err) => (
              <div
                key={err.filename}
                className="bg-red-50 border border-red-200 rounded-xl p-4"
              >
                <span className="font-medium text-gray-800">{err.filename}:</span>{" "}
        ...
        {extractResults !== null && !showDashboard && (
          <div className="mt-8 space-y-4">
            ...
            {extractResults.map((result) => (
              <div key={result.filename} className="bg-white rounded-xl shadow-sm p-6">
        ...
        {canRunAi && !showDashboard && (
          <div className="mt-8">
            {AUTH_ENABLED && !session?.user && !apiKey && (
              <div className="mb-4 bg-amber-50 border border-amber-200 rounded-xl p-4">
                <p className="text-amber-700 text-sm">
                  Enter your API key above, or{" "}
                  <Link href="/login?callbackUrl=/analyze" className="font-medium underline">
                    sign in
                  </Link>{" "}
                  to use server-side analysis.
                </p>
              </div>
            )}
            <button
          ...
          <div className="mt-8 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                ...
                <dd className="text-gray-900">{analysis.property.address ?? "\u2014"}</dd>
                <dt className="text-gray-500">Size</dt>
                <dd className="text-gray-900">
                  {analysis.property.sqm != null ? `${analysis.property.sqm} m\u00B2` : "\u2014"}
                </dd>
                <dt className="text-gray-500">Units</dt>
                <dd className="text-gray-900">{analysis.property.units ?? "\u2014"}</dd>
                <dt className="text-gray-500">Year Built</dt>
                <dd className="text-gray-900">{analysis.property.yearBuilt ?? "\u2014"}</dd>
                <dt className="text-gray-500">Type</dt>
                <dd className="text-gray-900">{analysis.property.type ?? "\u2014"}</dd>
              </dl>
            </div>
              ...
              <table className="w-full text-sm">
                <tbody>
                  {[
                    ...
                    ["Instandhaltungsr\u00FCcklage", analysis.financials.ruecklage],
                    ["Current Rent", analysis.financials.currentRent],
                    ["Expected Rent", analysis.financials.expectedRent],
                    ["Grunderwerbsteuer", analysis.financials.grunderwerbsteuer],
            ...
            {(analysis.protocols.upcomingRenovations.length > 0 ||
              ...
              <div className="bg-white rounded-xl shadow-sm p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-3">Protocol Findings</h3>
                <div className="space-y-4">
                  {analysis.protocols.upcomingRenovations.length > 0 && (
                    <div>
                      <h4 className="font-medium text-gray-700 mb-1">Upcoming Renovations</h4>
                  ...
                  {analysis.protocols.sonderumlagen.length > 0 && (
                    <div>
                      <h4 className="font-medium text-orange-700 mb-1">Sonderumlagen</h4>
                  ...
                  {analysis.protocols.maintenanceBacklog.length > 0 && (
                    <div>
                      <h4 className="font-medium text-gray-700 mb-1">Maintenance Backlog</h4>
                      <ul className="list-disc list-inside text-sm text-gray-600">
                  ...
                  {analysis.protocols.disputes.length > 0 && (
                    <div>
                      <h4 className="font-medium text-red-700 mb-1">Disputes</h4>
            ...
            {(analysis.wirtschaftsplan.annualBudget !== null ||
              ...
              <div className="bg-white rounded-xl shadow-sm p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-3">Wirtschaftsplan</h3>
                <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm mb-3">
                  <dt className="text-gray-500">Annual Budget</dt>
                  <dd className="text-gray-900">{formatEur(analysis.wirtschaftsplan.annualBudget)}</dd>
                  <dt className="text-gray-500">Reserve Fund Status</dt>
                  <dd className="text-gray-900">{analysis.wirtschaftsplan.reserveFundStatus ?? "\u2014"}</dd>
                </dl>
                {analysis.wirtschaftsplan.plannedMajorWorks.length > 0 && (
                  <div>
                    <h4 className="font-medium text-gray-700 mb-1 text-sm">Planned Major Works</h4>
                    <ul className="list-disc list-inside text-sm text-gray-600">
        ...
        {canGenerateReport && (
          <div className="mt-8">
            ...
            >
              {summaryLoading ? "Generating Full Report..." : "Generate Full Report"}
            </button>
          </div>
        )}
